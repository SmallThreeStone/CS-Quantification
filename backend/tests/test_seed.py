...
def test_seed_defaults_creates_thirty_test_items(db_session):
    ...
    assert len(items) == 30
    assert db_session.query(Item).filter(Item.is_active.is_(True)).count() == 30
    ...
    assert db_session.query(Item).filter(Item.pool_id == pools["观察池"].id).count() == 28
